                            ...
                            if [ -z "$CS_USERNAME" ] || [ -z "$CS_PASSWORD" ] || [ -z "$CS_REGISTRY" ] || [ -z "$CS_IMAGE_NAME" ] || [ -z "$CS_IMAGE_TAG" ] || [ -z "$CS_CLIENT_ID" ] || [ -z "$CS_CLIENT_SECRET" ] || [ -z "$FALCON_REGION" ] || [ -z "$PROJECT_PATH" ]; then
                                ...
                                if [ $? -eq 0 ]; then
                                    ...
                                    if [ $? -eq 0 ]; then
                                        ...
                                        # Find the actual scan output file
                                        echo "Looking for IaC scan results..."
                                        ls -lah "$WORKSPACE/iac_reports/" || echo "No iac_reports directory"

                                        # Try to find and copy the JSON file
                                        if [ -f "$WORKSPACE/iac_reports/iac_scan_results.json" ]; then
                                            cp "$WORKSPACE/iac_reports/iac_scan_results.json" "$WORKSPACE/iac_scan_results.json"
                                        elif [ -f "$WORKSPACE/iac_reports/scan_results.json" ]; then
                                            cp "$WORKSPACE/iac_reports/scan_results.json" "$WORKSPACE/iac_scan_results.json"
                                        else
                                            # Find any JSON file in the reports directory
                                            SCAN_FILE=$(find "$WORKSPACE/iac_reports" -name "*.json" -type f | head -1)
                                            if [ -n "$SCAN_FILE" ]; then
...
def get_iac_detections_from_api(token, base_url):
...
def generate_iac_report():
    ...
    # If no findings, try CrowdStrike API
    if len(findings) == 0:
        print("🌐 No local findings, fetching from CrowdStrike API...")
        ...
        if client_id and client_secret:
            ...
            if token:
                ...
                findings = get_iac_detections_from_api(token, base_url)
