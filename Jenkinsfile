...
def generate_iac_report():
    ...
    report_path = "/workspace/iac_scan_results.json"
    print(f"Looking for scan results at: {report_path}")
    if os.path.exists(report_path):
        print(f"📄 Reading local scan results from {report_path}")
        file_stat = os.stat(report_path)
        print(f"File size: {file_stat.st_size} bytes")
        print(f"File modified: {file_stat.st_mtime}")
        try:
...
.severity-informational{color:#61AFEF}
...
try:
    ...
    with open("/workspace/iac_assessment_report.html", "w") as f:
        f.write(html_report)
    print(f"✅ IaC Report generated successfully!")
...
'''
                        ...
                        // Debug: Verify scan results and delete old report
                        sh '''
                            echo "========================================"
                            echo "DEBUG: IaC Report Generation for Build #$BUILD_NUMBER"
                            echo "========================================"
                            echo "Current BUILD_NUMBER: $BUILD_NUMBER"
                            echo "Current JOB_NAME: $JOB_NAME"
                            echo "Workspace: $WORKSPACE"
                            echo ""

                            # Check if scan results file exists
                            if [ -f "$WORKSPACE/iac_scan_results.json" ]; then
                                echo "✅ Found iac_scan_results.json"
                                ls -lh "$WORKSPACE/iac_scan_results.json"
                                echo ""
                                echo "File modification time:"
                                stat -c '%y' "$WORKSPACE/iac_scan_results.json" 2>/dev/null || stat -f '%Sm' "$WORKSPACE/iac_scan_results.json"
                                echo ""
                                echo "First 5 lines of scan results:"
                                head -5 "$WORKSPACE/iac_scan_results.json"
                                echo ""
                            else
                                echo "❌ WARNING: iac_scan_results.json not found!"
                            fi

                            # Delete old HTML report to force fresh generation
                            if [ -f "$WORKSPACE/iac_assessment_report.html" ]; then
                                echo "🗑️  Deleting old iac_assessment_report.html"
                                rm -f "$WORKSPACE/iac_assessment_report.html"
                            fi
                        ...
                        // Verify report was generated with correct build number
                        sh '''
                            echo ""
                            echo "========================================"
                            ...
                            if [ -f "$WORKSPACE/iac_assessment_report.html" ]; then
                                ...
                                ls -lh "$WORKSPACE/iac_assessment_report.html"
                                echo ""
                                echo "Checking for Build #$BUILD_NUMBER in report:"
                                if grep -q "#$BUILD_NUMBER" "$WORKSPACE/iac_assessment_report.html"; then
                                    echo "✅ Report contains correct build number: #$BUILD_NUMBER"
                                else
                                    echo "❌ WARNING: Report does NOT contain build #$BUILD_NUMBER"
                                    echo "Build numbers found in report:"
                                    grep -o "#[0-9]*" "$WORKSPACE/iac_assessment_report.html" | head -5
                                fi
                            else
