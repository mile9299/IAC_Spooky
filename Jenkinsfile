        ...
        stage('Falcon Cloud Security Image Scan') {
            ...
            post {
                always {
                    script {
                        sh '''
                            # Find the CrowdStrike scan report file
                            echo "Looking for CrowdStrike scan files..."
                            find . -name "*crowdstrike*.json" -o -name "*report*.json" -o -name "*scan*.json" | head -20
                            ...
                            elif [ -f "CrowdstrikeSecurityReport.json" ]; then
                                echo "Found: CrowdstrikeSecurityReport.json"
                                cp CrowdstrikeSecurityReport.json container_scan.json
                            else
                                echo "No scan report found, creating empty file"
                                echo '{"Vulnerabilities": []}' > container_scan.json
                            fi

                            echo "Container scan file size:"
